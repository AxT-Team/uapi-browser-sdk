...
async function example() {
  ...
  } satisfies GetImageBingDailyRequest;

  try {
    const data = await api.getImageBingDaily(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
import type { GetImageBingDailyHistoryRequest } from 'uapi-browser-sdk-browser';
...
async function example() {
  ...
  const body = {
    // string | 壁纸日期，格式是 `YYYY-MM-DD`。传了以后会按日期精确查询，并且忽略 `page` 和 `page_size`。 (optional)
    date: 2026-04-05,
    // '4k' | '1080' | 返回图片的目标分辨率。可以传 `4k` 或 `1080`，不传时默认是 `4k`。 (optional)
    resolution: 1080,
    // number | 分页页码，必须是正整数。不传时默认是 `1`。只有在不传 `date` 时才生效。 (optional)
    page: 2,
    // number | 每页条数，必须是正整数。不传时默认是 `30`，最大是 `100`。只有在不传 `date` 时才生效。 (optional)
    pageSize: 10,
  } satisfies GetImageBingDailyHistoryRequest;

  try {
    const data = await api.getImageBingDailyHistory(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
async function example() {
  ...
  const body = {
    ...
    // number | 二维码图片的边长（正方形），单位是像素。有效范围是 256 到 2048 之间。 (optional)
    size: 512,
    // 'image' | 'json' | 'json_url' | 指定响应内容的格式。可选值为 `image`, `json`, `json_url`。 (optional)
    format: image,
    // boolean | 是否使用透明背景。启用后生成的 PNG 图片将具有 alpha 通道，背景透明。 (optional)
    transparent: true,
    // string | 二维码前景色（即二维码本身的颜色），使用十六进制格式。URL 中需要将 `#` 编码为 `%23`。 (optional)
    fgcolor: fgcolor_example,
    // string | 二维码背景色，使用十六进制格式。当 `transparent=true` 时此参数会被忽略。URL 中需要将 `#` 编码为 `%23`。 (optional)
...
> Blob postImageDecode(width, height, maxWidth, maxHeight, format, colorMode, fit, background, file, url)
...
import type { PostImageDecodeRequest } from 'uapi-browser-sdk-browser';
...
async function example() {
  ...
  const body = {
    ...
    maxHeight: 56,
    // 'bmp' | 'rgb565' | 'rgb888' | 输出格式。可以传 `bmp`、`rgb565` 或 `rgb888`，不传时默认是 `bmp`。 (optional)
    format: format_example,
    // 'RGB565' | 'RGB888' | BMP 输出的颜色模式。只有在 `format=bmp` 时才生效，可以传 `RGB565` 或 `RGB888`，不传时默认是 `RGB888`。 (optional)
    colorMode: colorMode_example,
    // 'contain' | 'cover' | 'fill' | 缩放模式。可以传 `contain`、`cover` 或 `fill`，不传时默认是 `contain`。当传 `cover` 或 `fill` 时，`width` 和 `height` 都要传。 (optional)
    fit: fit_example,
    // string | 背景色。可以传 `black`、`white` 或 `#RRGGBB`，不传时默认是 `black`。 (optional)
    background: background_example,
    // Blob | 要处理的图片文件。这个接口适合直接上传 JPG、JPEG、PNG、WebP、BMP 等常见格式。 (optional)
    file: BINARY_DATA_HERE,
    // string | 要处理的图片链接。适合不方便直接上传文件时使用。 (optional)
    url: url_example,
  } satisfies PostImageDecodeRequest;

  try {
    const data = await api.postImageDecode(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **format** | `bmp`, `rgb565`, `rgb888` | 输出格式。可以传 &#x60;bmp&#x60;、&#x60;rgb565&#x60; 或 &#x60;rgb888&#x60;，不传时默认是 &#x60;bmp&#x60;。 | [Optional] [Defaults to `&#39;bmp&#39;`] [Enum: bmp, rgb565, rgb888] |
| **colorMode** | `RGB565`, `RGB888` | BMP 输出的颜色模式。只有在 &#x60;format&#x3D;bmp&#x60; 时才生效，可以传 &#x60;RGB565&#x60; 或 &#x60;RGB888&#x60;，不传时默认是 &#x60;RGB888&#x60;。 | [Optional] [Defaults to `&#39;RGB888&#39;`] [Enum: RGB565, RGB888] |
| **fit** | `contain`, `cover`, `fill` | 缩放模式。可以传 &#x60;contain&#x60;、&#x60;cover&#x60; 或 &#x60;fill&#x60;，不传时默认是 &#x60;contain&#x60;。当传 &#x60;cover&#x60; 或 &#x60;fill&#x60; 时，&#x60;width&#x60; 和 &#x60;height&#x60; 都要传。 | [Optional] [Defaults to `&#39;contain&#39;`] [Enum: contain, cover, fill] |
| **background** | `string` | 背景色。可以传 &#x60;black&#x60;、&#x60;white&#x60; 或 &#x60;#RRGGBB&#x60;，不传时默认是 &#x60;black&#x60;。 | [Optional] [Defaults to `&#39;black&#39;`] |
| **file** | `Blob` | 要处理的图片文件。这个接口适合直接上传 JPG、JPEG、PNG、WebP、BMP 等常见格式。 | [Optional] [Defaults to `undefined`] |
...
async function example() {
  ...
  const body = {
    ...
    file: BINARY_DATA_HERE,
    // string | 图片的 URL 地址。如果同时提供 file 和 url，将优先使用 file。 (optional)
    url: url_example,
...
| Name | Type | Description  | Notes |
|------------- | ------------- | ------------- | -------------|
| **file** | `Blob` | 要检测的图片文件。支持 JPG、JPEG、PNG、GIF、WebP 格式，最大 20MB。 | [Optional] [Defaults to `undefined`] |
...
import type { PostImageOcrRequest } from 'uapi-browser-sdk-browser';
...
async function example() {
  ...
  const body = {
    ...
    // string | 是否返回文字坐标信息。请传 `true` 或 `false`，不传时默认是 `true`。 (optional)
  ...
  try {
    const data = await api.postImageOcr(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
