...
import type { GetGithubUserRequest } from 'uapi-browser-sdk-browser';
...
async function example() {
  ...
  const body = {
    // string | GitHub 用户名（必需符合 GitHub 命名规范：仅限字母、数字、连字符，最长 39 位）。
    user: torvalds,
    // boolean | 是否获取最近一年的贡献活动数据（如贡献图、时间线）。传入 true 开启，其他值均视为不开启。 (optional)
    activity: true,
    // 'all' | 'organization' | 活动数据范围。可选 all 或 organization。只有开启 activity 时才有意义。 (optional)
  ...
  try {
    const data = await api.getGithubUser(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
