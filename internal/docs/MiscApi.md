...
async function example() {
  ...
  const body = {
    ...
    nearbyLimit: 7,
    // boolean | 传 true 时，会过滤今天之前已经过去的节日。默认 false。 (optional)
    excludePast: true,
...
async function example() {
  ...
  const body = {
    ...
    extended: true,
    // boolean | 返回多天预报数据（最多7天），含每天的最高温度、最低温度、白天夜间天气、风向风力、日出日落等。 (optional)
    forecast: true,
    // boolean | 返回逐小时预报（24小时），含温度、天气、风向风速、湿度、降水概率等。 (optional)
    hourly: true,
    // boolean | 返回分钟级降水预报（仅国内城市），精确到2分钟。 (optional)
    minutely: true,
